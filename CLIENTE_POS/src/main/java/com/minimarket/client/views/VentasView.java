...
import com.minimarket.client.services.SucursalService;
...
public class VentasView extends JPanel implements MainWindow.Refreshable {
    ...
    // Sucursal
    private final JComboBox<Sucursal> sucursalCombo = new JComboBox<>();

    public VentasView(VentaService ventaSvc, ProductoService productoSvc,
                      ClienteService clienteSvc, SucursalService sucursalSvc) {
    ...
    private JPanel buildAcciones() {
        ...
        JPanel selectorsPanel = new JPanel(new GridLayout(2, 1, 0, 8));
        selectorsPanel.setOpaque(false);

        JPanel sucPanel = new JPanel(new BorderLayout(6, 0));
        sucPanel.setOpaque(false);
        JLabel sucLabel = new JLabel("Sucursal:");
        sucLabel.setFont(new Font("SansSerif", Font.PLAIN, 12));
        sucursalCombo.setFont(new Font("SansSerif", Font.PLAIN, 13));
        sucPanel.add(sucLabel, BorderLayout.NORTH);
        sucPanel.add(sucursalCombo, BorderLayout.CENTER);
        ...
        selectorsPanel.add(sucPanel);
        selectorsPanel.add(cliPanel);
        ...
        p.add(selectorsPanel, BorderLayout.NORTH);
    ...
    private void cargarSucursales() {
        int selectedId = sucursalSeleccionadaId();
        sucursalCombo.removeAllItems();
        List<Sucursal> sucursales = sucursalSvc.listar();
        for (Sucursal s : sucursales) {
            sucursalCombo.addItem(s);
            if (s.getId() == selectedId) sucursalCombo.setSelectedItem(s);
        }
        if (sucursalCombo.getItemCount() == 0) {
            status("No hay sucursales activas en el servidor.", Config.C_ERROR);
        }
    }
    ...
    private void registrarVenta() {
        ...
        int sucursalId = sucursalSeleccionadaId();

        if (sucursalId <= 0) {
            status("Seleccione una sucursal activa.", Config.C_WARNING);
            return;
        }

        Venta v = ventaSvc.crearVenta(cartItems, clienteId, sucursalId);
        if (v == null) {
            status("Error al registrar la venta. Verifique el stock.", Config.C_ERROR);
        } else {
            status("Venta #" + v.getId() + " registrada en " + nombreSucursalSeleccionada()
                + ". Total: " +
    ...
    private int sucursalSeleccionadaId() {
        Sucursal selected = (Sucursal) sucursalCombo.getSelectedItem();
        return selected != null ? selected.getId() : 0;
    }

    private String nombreSucursalSeleccionada() {
        Sucursal selected = (Sucursal) sucursalCombo.getSelectedItem();
        return selected != null ? selected.getNombre() : "Sucursal";
    }
